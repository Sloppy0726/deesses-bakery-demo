...
test('language toggle localizes current visible site sections', () => {
  ...
  assert.match(js, /function localizeCustomOrderPage\(\)[\s\S]*customCakeOrder[\s\S]*optionCopy[\s\S]*慶祝蛋糕[\s\S]*sendCustomOrder[\s\S]*orderingInformation/, 'Chinese mode should localize the custom cake order form, options, and ordering information');
